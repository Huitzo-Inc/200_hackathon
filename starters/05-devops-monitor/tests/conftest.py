"""
Test fixtures for the DevOps Monitor pack.

Provides mock implementations of all Huitzo SDK services (storage, LLM,
email, HTTP, Telegram) so tests run without external dependencies.
"""
...
class MockLLMBackend:
    ...
    async def complete(self, *, prompt: str, **kwargs: Any) -> str:
        ...
        return json.dumps(
            {
                "pattern": "Intermittent connection failures",
                "root_cause": "Possible DNS resolution issues",
                "recommendation": "Check DNS configuration and increase timeout",
                "confidence": "medium",
            }
        )
...
class MockHTTPBackend:
    """Mock HTTP backend that simulates endpoint responses."""

    responses: dict[str, Any] = field(default_factory=dict)
    calls: list[dict[str, Any]] = field(default_factory=list)
    fail_urls: set[str] = field(default_factory=set)
    timeout_urls: set[str] = field(default_factory=set)
    ...
    async def request(
        ...
        *,
        ...
        return self.responses.get(url, {"status": "ok", "status_code": 200})


@dataclass
class MockTelegramBackend:
    """Mock Telegram backend that records sent messages."""
...
@pytest.fixture
def telegram_backend() -> MockTelegramBackend:
...
def ctx(
    ...
    from huitzo_sdk import EmailClient, HTTPClient, LLMClient, TelegramClient
    ...
    context = Context(
        ...
        _telegram=TelegramClient(_backend=telegram_backend),
    )
